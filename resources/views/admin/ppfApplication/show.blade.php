...
@include('partials.alerts')
        ...
        <div class="main-content">
            <div class="container-fluid">
                <div id="personal-info" class="form-section">
                    ...
                    <h3 >Applicant's Information</h1>
                        <div style="text-align: end; color: #004d00; rgb(207, 207, 207)"><h3>{{ $PPFapplication->PPF_Association }}</h1></div>
                        <div style="text-align: end; color: #004d00; rgb(207, 207, 207)"><h3>{{ $PPFapplication->custom_id }}</h1></div>

                    </div>
                    <div class="card" style="padding-left: 20px; padding-bottom: 20px; padding-top: 20px; margin-top: 20px; ">
                        <p>Name: <b> {{ $PPFapplication->Applicants_name }}</b></p>
                        <p>Contact: <b>{{ $PPFapplication->Contact_No_1 }}</b></p>
                        <p>Address: <b>{{ $PPFapplication->Address1 }}</b></p>
                    </div>
                </div>
                ...
                <div id="personal-info" class="form-section">
                    ...
                    <p>Name: <b>{{ $PPFapplication->Drivers_name }}</b></p>
                    <p>Contact: <b>{{ $PPFapplication->Contact_No_2 }}</b></p>
                    <p>Address: <b> {{ $PPFapplication->Address_2 }}</b></p>
                    </div>
                </div>
                ...
                <div id="personal-info" class="form-section">
                    ...
                    <div class="card" style="padding-left: 20px; padding-bottom: 20px; padding-top: 20px;">
                        <p>Body no: <b>{{ $PPFapplication->Body_no }}</b></p>
                        <p>Plate no: <b>{{ $PPFapplication->Plate_no }}</b></p>
                        <p>Make: <b>{{ $PPFapplication->Make }}</b></p>
                        <p>Chassis no: <b>{{ $PPFapplication->Chassis_no }}</b></p>
                        <p>Engine no: <b> {{ $PPFapplication->Engine_no }}</b></p>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="mb-3">
                                    <select id="predefined-messages-{{ $PPFapplication->id }}" class="form-control" onchange="insertPredefinedMessage(this.value, {{ $PPFapplication->id }})">
                                ...
                                <textarea 
                                    id="remarks-{{ $PPFapplication->id }}" 
                                    class="form-control"
                                    rows="4" 
                                    style="resize: vertical; min-height: 100px;"
                                    placeholder="Add administrative remarks here..."
                                >{{ $PPFapplication->remarks }}</textarea>
                                <div class="d-flex justify-content-end mt-3">
                                    <button 
                                        onclick="updateRemarks({{ $PPFapplication->id }})"
                                        class="button-style"
            ...
            <div id="personal-info" class="form-section">
                ...
                <!-- Main Form -->
                <form action="{{ route('sticker-requirements.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="sticker_application_id" value="{{ $PPFapplication->id }}">
    ...
    uploadModal.addEventListener('show.bs.modal', (event) => {
        ...
        const stickerApplicationId = '{{ $PPFapplication->id }}'; // Pass it from the backend
        ...
        const actionUrl = `/admin/sticker-requirements/${stickerApplicationId}/upload/${requirementType}`;
...
<script>
    function updateRemarks(applicationId) {
        ...
        fetch(`/update-remarks/sticker/${applicationId}`, {
            method: 'POST',
            headers: headers,
            body: JSON.stringify({
                remarks: remarksText
