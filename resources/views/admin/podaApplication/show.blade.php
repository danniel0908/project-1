...
@include('partials.alerts')
        ...
        <div class="main-content">
            <div class="container-fluid">
                ...
                <div id="personal-info" class="form-section">
                    ...
                    <h3 >Applicant's Information</h1>
                        <div style="text-align: end; color: #004d00; rgb(207, 207, 207)"><h3>{{ $PODAapplication->custom_id }}</h1></div>
                    </div>
                    <div class="card" style="padding-left: 20px; padding-bottom: 20px; padding-top: 20px; margin-top: 20px; ">
                        <p>Name: <b> {{ $PODAapplication->Applicants_name }}</b></p>
                        <p>Contact: <b>{{ $PODAapplication->Contact_No_1 }}</b></p>
                        <p>Address: <b>{{ $PODAapplication->Address1 }}</b></p>
                    </div>
                </div>
                ...
                <div id="personal-info" class="form-section">
                    ...
                    <p>Name: <b>{{ $PODAapplication->Drivers_name }}</b></p>
                    <p>Contact: <b>{{ $PODAapplication->Contact_No_2 }}</b></p>
                    <p>Address: <b> {{ $PODAapplication->Address_2 }}</b></p>
                    </div>
                </div>

                <div id="personal-info" class="form-section">
                    <h3 style="color: #004d00; border-bottom: 1px solid rgb(207, 207, 207); padding-left: 20px; padding-bottom: 20px;">Pedicab's Description</h1>
                    <div class="card" style="padding-left: 20px; padding-bottom: 20px; padding-top: 20px;">
                        <p>Sticker no: <b>{{ $PODAapplication->Sticker_no }}</b></p>
                        <p>Unit no: <b>{{ $PODAapplication->Unit_no1 }}</b></p>
                        <p>Unit no: <b>{{ $PODAapplication->Unit_no2 }}</b></p>
                        <p>Unit no: <b>{{ $PODAapplication->Unit_no3 }}</b></p>
                        <p>Unit no: <b> {{ $PODAapplication->Unit_no4 }}</b></p>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="mb-3">
                                    <select id="predefined-messages-{{ $PODAapplication->id }}" class="form-control" onchange="insertPredefinedMessage(this.value, {{ $PODAapplication->id }})">
                                ...
                                <textarea 
                                    id="remarks-{{ $PODAapplication->id }}" 
                                    class="form-control"
                                    rows="4" 
                                    style="resize: vertical; min-height: 100px;"
                                    placeholder="Add administrative remarks here..."
                                >{{ $PODAapplication->remarks }}</textarea>
                                <div class="d-flex justify-content-end mt-3">
                                    <button 
                                        onclick="updateRemarks({{ $PODAapplication->id }})"
                                        class="button-style"
            ...
            <div id="personal-info" class="form-section">
                ...
                <!-- Main Form -->
                <form action="{{ route('poda-requirements.update') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="poda_application_id" value="{{ $PODAapplication->id }}">
                    <table class="table table-bordered" style="width: 100%; margin-top: 20px;">
                        ...
                        <tbody>
                            @foreach([
                                    ...
                                    'Picture' => 'Pictures wearing PODA uniform (2 Original Copies)',
    ...
    uploadModal.addEventListener('show.bs.modal', (event) => {
        ...
        const podaApplicationId = '{{ $PODAapplication->id }}'; // Pass it from the backend
        ...
        const actionUrl = `/admin/poda-requirements/${podaApplicationId}/upload/${requirementType}`;
    ...
    fetch(`/update-remarks/poda/${applicationId}`, {
        method: 'POST',
        headers: headers,
        body: JSON.stringify({
            remarks: remarksText
