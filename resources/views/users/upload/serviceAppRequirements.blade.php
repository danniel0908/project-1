    ...
    <div style="display: flex; justify-content: center; align-items: center; min-height: 100vh; margin-top: 20px;">
        <div class="container">
            <div class="sidebar">
                ...
                <nav>
                    <ul>
                    <li><a href="{{ route('service.edit', $Application->id) }}" style="color: #004d00;" >1.) Personal Information</a></li>
                    <li><a href="{{ route('schedule.create', ['serviceApplicationId' => $Application->id]) }}" style="color: #004d00;">2.) Schedule</a></li>
                    <li><a href="{{ route('service.upload', ['service_application_id' => $Application->id]) }}" style="color: #004d00;">3.) Requirements</a></li>
                    <li>
                    @php
                            $approvedRequirementsCount = \App\Models\privateServiceRequirements::where('private_service_id', $Application->id)
                        ...
                        @if($hasAllRequirementsApproved)
                            <a href="{{ route('service.payment', ['id' => $Application->id]) }}" 
            ...
            <div class="main-content">
                ...
                <table>
                    ...
                    <tbody>
                        <form action="{{ route('service.upload' ,['service_application_id' => $Application->id]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <tr>
                            ...
                            <tr>
                                <td>Professional driver's license (1Photocopy)
                                <br>    
                                 @foreach($files as $file)
                                    @if($file->requirement_type == 'license')
                                        @if ($file->status == 'approved')
                                            <p class="status approved"><i class="fa fa-check-circle"></i> Approved</p>                                              
                                        @elseif ($file->status == 'reject')
                                            <p class="status invalid" onclick="showRemarks('{{ $file->remarks }}')" style="cursor: pointer;">
                                ...
                                <td>
                                    @if($files->isEmpty() || !$files->contains('requirement_type', 'license'))
                                    <p>No files uploaded for this requirement.</p>
                                    <td>
                                        <input type="file" class="form-control" name="files[]" id="file2">
                                        <input type="hidden" name="requirement_type[]" value="license">
                                    </td>
                                    @else
                                        @foreach($files as $file)
                                            @if($file->requirement_type == 'license')
                            ...
                            <tr>
                                <td>Deed of Sale or Deed of Conveyance/Transfer (1 Photocopy)
                                <br>    
                                 @foreach($files as $file)
                                    @if($file->requirement_type == 'Deed_of_Sale')
                                        @if ($file->status == 'approved')
                                            <p class="status approved"><i class="fa fa-check-circle"></i> Approved</p>                                              
                                        @elseif ($file->status == 'reject')
                                            <p class="status invalid" onclick="showRemarks('{{ $file->remarks }}')" style="cursor: pointer;">
                                ...
                                <td>
                                    @if($files->isEmpty() || !$files->contains('requirement_type', 'Deed_of_Sale'))
                                    <p>No files uploaded for this requirement.</p>
                                    <td>
                                        <input type="file" class="form-control" name="files[]" id="file2">
                                        <input type="hidden" name="requirement_type[]" value="Deed_of_Sale">
                                    </td>
                                    @else
                                        @foreach($files as $file)
                                            @if($file->requirement_type == 'Deed_of_Sale')
                            ...
                            <tr>
                                <td>Insurance Coverage for Third Party Liability (1 Photocopy)
                                <br>    
                                 @foreach($files as $file)
                                    @if($file->requirement_type == 'Insurance')
                                        @if ($file->status == 'approved')
                                            <p class="status approved"><i class="fa fa-check-circle"></i> Approved</p>                                              
                                        @elseif ($file->status == 'reject')
                                            <p class="status invalid" onclick="showRemarks('{{ $file->remarks }}')" style="cursor: pointer;">
                                ...
                                <td>
                                    @if($files->isEmpty() || !$files->contains('requirement_type', 'Insurance'))
                                    <p>No files uploaded for this requirement.</p>
                                    <td>
                                        <input type="file" class="form-control" name="files[]" id="file2">
                                        <input type="hidden" name="requirement_type[]" value="Insurance">
                                    </td>
                                    @else
                                        @foreach($files as $file)
                                            @if($file->requirement_type == 'Insurance')
                            ...
                            <tr>
                                <td>Pictures wearing TODA uniform (2 Original Copies)
                                <br>    
                                 @foreach($files as $file)
            ...
            <p style ="text-align: center">Note: If the requirement is marked as invalid, click on the 'Invalid' mark to check the remarks.</p>
                </div>
            </div>
        </div>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    fileUpdateForm.action = `/service/update-file/${fileId}`;
